...
def build_panel() -> tuple[pd.DataFrame, list[str]]:
    """Price panel + labels + sector/market/catalyst features (engine_replay.py reuses this)."""
...
def fit_horizon(df, ALL_FEATS, label_col, train_yrs, oof_yrs):
...
def main() -> None:
    ...
    df, ALL_FEATS = build_panel()
    per_horizon = {}
    for H, _ in HORIZONS:
        print(f"\n>> horizon {H}d")
        lgbm, xgbm, oof = fit_horizon(df, ALL_FEATS, f"label_h{H}", train_yrs=[2020, 2021, 2022, 2023], oof_yrs=[2024, 2025])
