#!/usr/bin/env bash
# FRIDAY-ENTRY RUN (entry 2026-08-28 pre-open AMO) — checklist-by-checklist, data through Thu Aug-27 close.
...
log "☐ 1. PRICES (through Thu Aug-27) + continuity"
run f1_prices /usr/bin/python3 src/agentic/refresh_prices.py || exit 1
qc "prices Thu Aug-27 + continuous" "
...
mx = pd.to_datetime(px['trade_date']).max().date(); print('max', mx); assert str(mx) >= '2026-08-27'"

log "☐ 2. CORPORATE ACTIONS (both stores)"
/usr/bin/python3 - > "$LOG_DIR/${TS}_f2_ca.log" 2>&1 << 'PYEOF'
...
new = load_corporate_actions_from_nse(NseCorporateActionsFetchConfig(
    ...
    start_date=date(2026, 8, 17), end_date=date.today()))
...
PYEOF
if [ $? -eq 0 ]; then log "  ✅ f2_ca"; head -3 "$LOG_DIR/${TS}_f2_ca.log"; else log "  ❌ f2_ca FAILED"; tail -5 "$LOG_DIR/${TS}_f2_ca.log"; exit 1; fi

log "☐ 3. ANNOUNCEMENTS (real store)"
/usr/bin/python3 - > "$LOG_DIR/${TS}_f3_ann.log" 2>&1 << 'PYEOF'
import sys; sys.path.insert(0, '.')
from datetime import date
from pathlib import Path
import pandas as pd
...
new = load_announcements_from_nse(NseAnnouncementFetchConfig(
    ...
    start_date=date(2026, 8, 20), end_date=date.today()))
...
PYEOF
if [ $? -eq 0 ]; then log "  ✅ f3_ann"; else log "  ❌ f3_ann FAILED"; exit 1; fi
...
assert str(d.max().date()) >= '2026-08-26'"

log "☐ 4. NEWS EVENTS"
run f4_news /usr/bin/python3 src/agentic/build_news_event_features.py || exit 1

log "☐ 5. MACRO (FX + commodities + global/FRED)"
run f5a_fx /usr/bin/python3 src/agentic/fetch_forex_macro.py || exit 1
run f5b_cmdty /usr/bin/python3 src/agentic/fetch_commodity_prices.py || exit 1
run f5c_global /usr/bin/python3 src/agentic/fetch_global_macro.py || log "  ⚠ yahoo leg (expected) — FRED fallback ran inside"

log "☐ 6. INDUSTRY + BREADTH + ENRICHMENT + PANEL"
run f6a_industry /usr/bin/python3 src/agentic/fetch_industry_indicators.py || exit 1
run f6b_breadth /usr/bin/python3 src/agentic/fetch_market_breadth.py || log "  ⚠ breadth non-fatal"
for s in fetch_fii_dii fetch_block_deals fetch_news_rss score_sentiment fetch_global_macro_sentiment; do
  /usr/bin/python3 src/agentic/$s.py > "$LOG_DIR/${TS}_f6c_$s.log" 2>&1 && log "  ✅ $s" || log "  ⚠ $s failed (non-fatal)"
done
run f6d_panel /usr/bin/python3 src/agentic/build_macro_panel.py || exit 1
qc "macro cols non-null fresh" "
import pandas as pd
m = pd.read_parquet('data/derived/macro_panel.parquet'); m['trade_date']=pd.to_datetime(m['trade_date'])
for c, floor in [('usdinr','2026-08-25'),('brent','2026-08-21'),('us_10y','2026-08-21'),('dxy','2026-08-21'),('us_vix','2026-08-21'),('spx','2026-08-21')]:
    s = m.dropna(subset=[c]); d = str(s['trade_date'].max().date()); print(c, d)
    assert d >= floor, f'{c} stale {d} < {floor}'"

log "☐ 7. RL: self-score + miss learner + classifier"
run f7a_score /usr/bin/python3 src/agentic/score_basket_outcomes.py || log "  ⚠ score non-fatal"
tail -4 "$LOG_DIR/${TS}_f7a_score.log" 2>/dev/null
/usr/bin/python3 src/agentic/miss_learner.py --entry 2026-08-24 --exit 2026-08-27 --top-n 20 > "$LOG_DIR/${TS}_f7b_miss.log" 2>&1 \
  && log "  ✅ miss_learner (Aug-24→27 partial window)" || log "  ⚠ miss_learner failed"
run f7c_classifier /usr/bin/python3 src/agentic/train_missed_winner_classifier.py || exit 1
qc "classifier sane" "
import re
t = open('$LOG_DIR/${TS}_f7c_classifier.log').read()
...
log "☐ 8. ENGINES x5"
/usr/bin/python3 src/agentic/compare_short_horizons.py    > "$LOG_DIR/${TS}_f8_cs.log"   2>&1 & P1=$!
/usr/bin/python3 src/agentic/find_high_conviction.py      > "$LOG_DIR/${TS}_f8_hc.log"   2>&1 & P2=$!
/usr/bin/python3 src/agentic/find_multibagger_today.py    > "$LOG_DIR/${TS}_f8_mb.log"   2>&1 & P3=$!
/usr/bin/python3 src/agentic/run_multi_horizon.py         > "$LOG_DIR/${TS}_f8_mh.log"   2>&1 & P4=$!
/usr/bin/python3 src/agentic/find_180d_frontier_honest.py > "$LOG_DIR/${TS}_f8_180d.log" 2>&1 & P5=$!
for np in "cs:$P1" "hc:$P2" "mb:$P3" "mh:$P4" "180d:$P5"; do
  n=${np%%:*}; p=${np##*:}
  wait $p && log "  ✅ engine $n" || log "  ❌ engine $n failed"
done
qc "all 5 engine outputs fresh (mtime today or later)" "
from pathlib import Path
...
for f in ['data/derived/compare_short_horizons.parquet','data/derived/high_conviction_predictions.parquet',
    ...
    assert m >= date(2026,8,27), f'{f} stale mtime {m}'
    print(f.split('/')[-1], m)"

log "☐ 9. GATE (27 checks) → BASKET → FORENSICS → SHADOW"
/usr/bin/python3 src/agentic/emit_freshness_status.py
run f9_gate /usr/bin/python3 src/agentic/verify_freshness.py || { log "🛑 GATE FAILED"; grep FAIL "$LOG_DIR/${TS}_f9_gate.log"; exit 1; }
run f9_basket /usr/bin/python3 src/agentic/generate_hybrid_basket.py || exit 1
qc "basket contract (newest file, midnight-safe)" "
import json, glob, os
f = max(glob.glob('live_predictions/*_15d5pct.json'), key=os.path.getmtime)
b = json.load(open(f))
assert b['data_through'] >= '2026-08-27', f'basket data_through stale: {b[\"data_through\"]}'
...
print('contract OK:', f, 'data_through', b['data_through'])"
run f10_forensics /usr/bin/python3 src/agentic/tape_forensics.py || log "  ⚠ forensics non-fatal"
grep -E "SEVERITY-3|sev3" "$LOG_DIR/${TS}_f10_forensics.log" 2>/dev/null || log "  (no severity-3 flags in log)"
run f11_shadow /usr/bin/python3 src/agentic/lean_shadow_check.py || log "  ⚠ shadow non-fatal"
log "════════ FRIDAY RUN COMPLETE ════════"
