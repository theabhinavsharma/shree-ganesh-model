...
def run_quick_compare(
    *,
    ...
    champion_metrics = _load_champion_metrics(champion_metrics_path, challenger.universe_name)
    ...
    write_dataframe_manifest(
        challenger_path,
        challenger_df,
        generated_by="src/analysis/day1_random_forest_quick_compare.py",
        as_of_date=evaluation_end_date,
        extra_notes=["Challenger is Random Forest on the same day-1 target and yearly walk-forward folds."],
    )
    write_dataframe_manifest(
        comparison_path,
        comparison_df,
    ...
    write_report_directory_readme(
        output_dir,
        title="Day 1 Random Forest Quick Compare",
        intro_lines=[
...
def _fit_predict_regressor(
    *,
    ...
    model.fit(train_x, train_y)
    return np.clip(model.predict(test_x), -0.15, 0.25).astype(np.float32)


def _load_champion_metrics(path: Path, universe_name: str) -> dict[str, float | str]:
    metrics = pd.read_csv(path)
    row = metrics.loc[metrics["universe_name"] == universe_name]
    if row.empty:
        raise ValueError(f"Universe {universe_name} not found in champion metrics file {path}.")
...
def _compare_rows(champion: dict[str, object], challenger: dict[str, object]) -> dict[str, object]:
    return {
        "universe_name": challenger["universe_name"],
        "top10_precision_champion": champion["top10_precision_5pct"],
        "top10_precision_random_forest": challenger["top10_precision_5pct"],
        "delta_top10_precision": float(challenger["top10_precision_5pct"]) - float(champion["top10_precision_5pct"]),
        "precision_lift_champion": champion["precision_lift_top10"],
        "precision_lift_random_forest": challenger["precision_lift_top10"],
        "delta_precision_lift": float(challenger["precision_lift_top10"]) - float(champion["precision_lift_top10"]),
        "top10_recall_champion": champion["top10_recall"],
        "top10_recall_random_forest": challenger["top10_recall"],
        "delta_top10_recall": float(challenger["top10_recall"]) - float(champion["top10_recall"]),
        "top10_mean_return_champion": champion["top10_mean_return_mean"],
        "top10_mean_return_random_forest": challenger["top10_mean_return_mean"],
        "delta_top10_mean_return": float(challenger["top10_mean_return_mean"]) - float(champion["top10_mean_return_mean"]),
        "top10_median_return_champion": champion["top10_median_stock_return_median"],
        "top10_median_return_random_forest": challenger["top10_median_stock_return_median"],
        "delta_top10_median_return": float(challenger["top10_median_stock_return_median"]) - float(champion["top10_median_stock_return_median"]),
        "top10_p75_return_champion": champion["top10_p75_stock_return_median"],
        "top10_p75_return_random_forest": challenger["top10_p75_stock_return_median"],
        "delta_top10_p75_return": float(challenger["top10_p75_stock_return_median"]) - float(champion["top10_p75_stock_return_median"]),
        "winner_metric": "random_forest" if float(challenger["top10_precision_5pct"]) > float(champion["top10_precision_5pct"]) else "champion",
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Quick Random Forest versus champion compare for day-1 5 percent target.")
    parser.add_argument("--config", type=Path, default=Path("configs/ml_research.yaml"))
    parser.add_argument("--champion-metrics", type=Path, default=Path("reports/day1_5pct_model_20260413/universe_metrics.csv"))
    ...
    parser.add_argument("--universe", default="liquid_5cr_plus")
    parser.add_argument("--top-n", type=int, default=10)
    parser.add_argument("--classifier-trees", type=int, default=60)
    parser.add_argument("--regressor-trees", type=int, default=40)
